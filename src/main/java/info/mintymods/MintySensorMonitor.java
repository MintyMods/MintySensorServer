...
public class MintySensorMonitor {
	...
	public MsmMonitorResponse getSensorsData(MsmMonitorRequest request) {
		...
		String exe = MintyFileUtils.getPathToMintySensorMonitor();
		try (ByteArrayOutputStream stream = new ByteArrayOutputStream()) {
			...
			return MintyJsonUtils.getMsmMonitorResponse(stream.toString());
		} catch (IOException | InterruptedException e) {
			throw new RuntimeException("Error launching Process Builder for MSM", e);
		}
	}

	public static String getJsonRequest(MsmMonitorRequest request) {
		try {
			return MintyJsonUtils.getMapper().writeValueAsString(request);
